...
def setup_lc_args(argv):
    ...
    parser.add_argument("--bpg_path", type=str, help="Corresponding BPG filepath to calculate target bpp")
    ...
    if args.codec not in TRADITIONAL_CODECS:
        ...
        for dir in subdirs:
            ...
            for file in os.listdir(dir):
                ...
                statement = ('python TCM/bin/encoder.py ' + input_image + ' --target_bpp ' +
                             target_bpp + ' -o ' + out_filepath)
        ...
        for dir in subdirs:
            ...
            if not os.path.exists(out_dir):
                os.mkdir(out_dir)
            nargs = [(codec, i, f, out_dir + '/' + f.rsplit('/', 1)[1][:-4] + '.bin', q) for i, q in
                     enumerate(tc_args.qps) for f in img_list]
