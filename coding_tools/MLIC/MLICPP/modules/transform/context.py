...
class LocalContext(nn.Module):
    def __init__(
        ...
        self.unfold = nn.Unfold(
            kernel_size=window_size, stride=1, padding=(window_size - 1) // 2
        )
        ...
        self.mlp = MLP(
            in_dim=dim * 2, hidden_dim=int(dim * 2 * mlp_ratio), out_dim=dim * 2
        )
        self.norm1 = nn.LayerNorm(dim)
        self.norm2 = nn.LayerNorm(dim * 2)
        self.register_buffer(
            "relative_position_index", build_position_index((window_size, window_size))
        )
        self.attn_mask = None
    ...
    def update_resolution(self, H, W, device, mask=None):
        ...
        if self.H != H or self.W != W:
            ...
            qk_windows = qk_windows.view(
                1, H * W, 2, 1, self.window_size, self.window_size
            ).permute(2, 0, 1, 3, 4, 5)
            q_windows, k_windows = qk_windows[0], qk_windows[1]
            q = q_windows.reshape(
                1, H * W, 1, self.window_size * self.window_size
            ).permute(0, 1, 3, 2)
            k = k_windows.reshape(
                1, H * W, 1, self.window_size * self.window_size
            ).permute(0, 1, 3, 2)
    ...
    def forward(self, x):
        ...
        qkv_windows = qkv_windows.view(
            B, L, 3, C, self.window_size, self.window_size
        ).permute(2, 0, 1, 3, 4, 5)
        ...
        q = q_windows.reshape(
            B, L, self.head_dim, self.num_heads, self.window_size * self.window_size
        ).permute(0, 1, 3, 4, 2)
        k = k_windows.reshape(
            B, L, self.head_dim, self.num_heads, self.window_size * self.window_size
        ).permute(0, 1, 3, 4, 2)
        ...
        x = (
            (attn @ v)
            .reshape(
                B, L, self.num_heads, self.window_size, self.window_size, self.head_dim
            )
            .permute(0, 1, 3, 4, 2, 5)
        )
...
class LinearGlobalIntraContext(nn.Module):
    def __init__(self, dim=32, num_heads=2) -> None:
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        ...
        self.mlp = nn.Sequential(
            ...
            nn.GELU(),
            nn.Conv2d(
                dim * 4, dim * 4, kernel_size=3, stride=1, padding=1, groups=dim * 4
            ),
            nn.GELU(),
    ...
    def forward(self, x1, x2):
        ...
        queries = ckbd_nonanchor_sequeeze(self.queries(x1_na)).reshape(
            B, self.dim, H * W // 2
        )
        ...
        for i in range(self.num_heads):
            ...
            key = ckbd_anchor_unsequeeze(key.reshape(B, head_dim, H, W // 2)).reshape(
                B, head_dim, H * W
            )
            value = ckbd_anchor_unsequeeze(
                value.reshape(B, head_dim, H, W // 2)
            ).reshape(B, head_dim, H * W)
            query = ckbd_nonanchor_unsequeeze(
                query.reshape(B, head_dim, H, W // 2)
            ).reshape(B, head_dim, H * W)
...
class LinearGlobalInterContext(nn.Module):
    def __init__(self, dim=32, out_dim=64, num_heads=2) -> None:
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        self.keys = nn.Sequential(
        ...
        self.reprojection = nn.Conv2d(
            dim, out_dim * 3 // 2, kernel_size=5, stride=1, padding=2
        )
        self.mlp = nn.Sequential(
        ...
        self.skip = nn.Conv2d(
            out_dim * 3 // 2, out_dim, kernel_size=1, stride=1, padding=0
        )
