    ...
    def forward(self, x):
        ...
        for idx, y_slice in enumerate(y_slices):
            ...
            if idx == 0:
                ...
                slice_nonanchor = (
                    ste_round(slice_nonanchor - means_nonanchor) + means_nonanchor
                )
...
class MLICPlusPlusVBR(MLICPlusPlus):
    def __init__(self, config, **kwargs):
        super().__init__(config, **kwargs)

        self.q_basic = nn.Parameter(torch.ones((1, config.M, 1, 1)))
    ...
    def load_state_dict(self, state_dict, verbose=True, **kwargs):
        # Skip nonexisting args in checkpoint
        sd = self.state_dict()
        for skey in sd:
