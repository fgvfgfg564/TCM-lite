...
class ModelEngine(nn.Module):
    ...
    def __init__(self, model_name) -> None:
        ...
        self.lmbda_min = torch.tensor(
            0.0025 * np.exp(int(model_name[8])), dtype=torch.half, device="cuda"
        )
        self.lmbda_max = torch.tensor(
            0.0025 * np.exp(int(model_name[8]) + 1), dtype=torch.half, device="cuda"
        )
        self.cuda()
    ...
    def compress_block(self, img_block, q_scale):
        ...
        bit_stream_y, bit_stream_z = self.i_frame_net.compress(img_block, lmbda.half())[
            "strings"
        ]
