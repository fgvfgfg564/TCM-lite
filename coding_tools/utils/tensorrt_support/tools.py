...
def compile(model: torch.nn.Module, output_folder):
    ...
    for name, child in model.named_children():
        if (
            hasattr(child, "tensorrt_compilable")
            and child.tensorrt_compilable
            and hasattr(child, "input_shape")
        ):
            input_shape = child.input_shape
            print(
                f"Compiling module: {name} into TensorRT TorchScript; input_shape={input_shape}"
            )
