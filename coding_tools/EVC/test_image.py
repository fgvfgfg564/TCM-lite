...
def parse_args():
    ...
    parser.add_argument(
        "--ec_thread", type=str2bool, nargs="?", const=True, default=False
    )
...
def run_test(i_frame_net, args, device):
    ...
    with torch.no_grad():
        ...
        bin_path = os.path.join(args["bin_folder"], "0.bin") if write_stream else None

        result = i_frame_net.encode_decode(
            x_padded,
            args["i_frame_q_scale"],
            bin_path,
        ...
        if verbose >= 1:
            print(
                f"{os.path.basename(args['img_path'])}, rate: {args['rate_idx']}, "
        ...
        if save_decoded_frame:
            ...
            save_path = os.path.join(
                ...
                f"{os.path.basename(args['img_path'])}.png",
            )
            save_torch_image(x_hat, save_path)
            os.rename(
                args["decoded_frame_folder"],
                args["decoded_frame_folder"] + f"/../{folder_name}",
            )
...
def encode_one(args, device):
    ...
    if args["save_decoded_frame"]:
        decoded_frame_folder = os.path.join(
            args["decoded_frame_path"], sub_dir_name, str(args["rate_idx"])
        )
...
def prepare_args(args, config, ds_name, img_name, rate_idx, i_frame_q_scales):
    ...
    if cur_args["img_folder"] == 1:
        cur_args["img_path"] = os.path.join(
            args.root_path, config[ds_name]["base_path"], img_name
        )
    else:
        cur_args["src_height"] = config[ds_name]["sequences"][img_name]["height"]
        cur_args["src_width"] = config[ds_name]["sequences"][img_name]["width"]
        cur_args["dataset_path"] = os.path.join(
            args.root_path, config[ds_name]["base_path"]
        )
...
def main():
    ...
    if args.i_frame_q_scales is not None:
        ...
        print(
            f"testing {rate_num} rate points with pre-defined intra y q_scales: ",
            end="",
        )
    ...
    for ds_name in config:
        ...
        if config[ds_name].get("img_folder", 0) == 1:
            imgs = os.listdir(
                os.path.join(args.root_path, config[ds_name]["base_path"])
            )
            imgs.sort()
        ...
        for img_name in imgs:
            ...
            for rate_idx in range(rate_num):
                cur_args = prepare_args(
                    args, config, ds_name, img_name, rate_idx, i_frame_q_scales
                )
                count_frames += 1

                if worker_num > 1:
                    obj = threadpool_executor.submit(worker, args.cuda, cur_args)
                    objs.append(obj)
                else:
