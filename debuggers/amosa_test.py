...
if __name__ == "__main__":
    ...
    config.archive_hard_limit = 1
    config.archive_soft_limit = 2
    config.archive_gamma = 2
    config.clustering_max_iterations = 5
    config.hill_climbing_iterations = 10
    config.initial_temperature = 1.0
    config.cooling_factor = 0.9
    config.annealing_iterations = 1
    ...
    optimizer.run(problem, termination, remove_checkpoints=False)
